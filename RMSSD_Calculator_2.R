...
#Retrieve paths for data and the ID's of subjects
...
#init number of subjects
Number_of_subjects <- (length(files)/2)
...
output <- foreach(m = 1:Number_of_subjects,.combine='comb', .multicombine=TRUE,
...
stopCluster(cl)  
  ...
  output_RMSSD_Store_filtered <- as.data.frame(l2,stringsAsFactors=FALSE)
      ...
      for (k in seq(1,Number_of_subjects,1)){
  ...
  title("Mean RMSSD across subjects associated with Stimulation Events > 15 sec", cex.lab=0.75)
